...
impl INode for Instance {
    ...
    fn process(&mut self, _delta: f64) {
                    ...
                    if (*x - nextx).abs() == 1 || (*y - nexty).abs() == 1 {
                        if !col_array.get_at(nextx, nexty) {
                            *x = nextx; *y = nexty; *speed = nextspeed;
                            ...
                            godot_print!("Move into wall by {}: {}, {}, {}", &p.data.name, nextx, nexty, nextspeed);
                        }
                    }
        ...
        self.players.retain(|_, p| !p.borrow().data.is_null());
...
impl Instance {
    ...
    pub fn spawn_player(&mut self, player: Rc<RefCell<Player>>, x: i32, y: i32, net_id: i32) {
    ...
    pub fn despawn_player(&mut self, net_id: i32) {
